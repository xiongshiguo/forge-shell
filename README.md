...
⚡ **V2.2 进化引擎版** — 经验采集 → 反思提炼 → SOP 匹配，全闭环运转

- [x] 阶段 0-5：基础架构、TUI/Web UI、四级缓存、工具沙箱、记忆系统
- [x] 阶段 6：性能优化 + 多平台编译
- [x] 阶段 7：进化引擎（经验采集、反思引擎、天工阁 SOP 库）
- [x] V2 Web UI：紫金配色浏览器界面，SSE 流式对话，首次启动输入 Key
- [x] V2.1：接通真实 DeepSeek API，修复假数据，Key 持久化
- [x] V2.2：进化闭环 + 启动版本检测 + 社区网站动态化
...
| 🧠 模型路由 | 简单→Flash (成本 1/10)，复杂→Pro |
| 🛡️ 沙箱 | 命令白名单、路径穿透检测、自动备份 |
